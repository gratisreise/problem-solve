...
접근 아이디어
* DFS:
    * 각 위치에서 상하좌우로 이동하며 방문 가능한 알파벳 탐색.
    * 백트래킹을 통해 방문한 알파벳을 해제하여 모든 경로 탐색.
    * 각 경로의 방문 칸 수로 최대값 갱신.
* 상태 관리:
    * board: 격자 상태 (알파벳).
    * visited: 방문한 알파벳 집합.
    * max_count: 최대 방문 칸 수.
* 구현 전략:
    ...
    * 방문하지 않은 알파벳으로 이동하며 경로 길이 계산.
    * 백트래킹으로 다른 경로 탐색, 최대값 갱신.
1. 입력 처리
...
* visited = set([board[0][0]]): 시작 위치 (0,0)의 알파벳을 방문 집합에 추가.
* dfs(0, 0, visited, 1): DFS 시작, 초기 위치 (0,0), 시작 칸 포함(count=1).
3. DFS 함수
* dfs(y, x, visited, count):
    ...
    * 4방향 탐색:
        ...
        * 조건 만족 시:
            ...
            * dfs(ny, nx, visited, count + 1): 재귀 호출.
...
시간 복잡도
* 상태: 위치 (R * C) × 방문 알파벳 조합.
* 최악: 각 위치에서 4방향, 최대 20×20 격자, 알파벳 26개.
* 이론적 최악: O(4^(R * C)).
* 실제: R * C ≤ 400, 경로 길이 ≤ min(R * C, 26) → 백트래킹으로 효율적 (1초 내 통과).
